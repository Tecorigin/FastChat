...
def get_gpu_memory(max_gpus=None):
    ...
    num_gpus = (
        ...
        if max_gpus is None
        else min(max_gpus, torch.sdaa.device_count())
    )

    for gpu_id in range(num_gpus):
        with torch.sdaa.device(gpu_id):
            device = torch.sdaa.current_device()
            gpu_properties = torch.sdaa.get_device_properties(device)
            total_memory = gpu_properties.total_memory / (1024**3)
            allocated_memory = torch.sdaa.memory_allocated() / (1024**3)
